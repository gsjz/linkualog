...
export const PREFILL_VOCAB_MANUAL_EVENT = 'prefill-vocab-manual';
...
export default function VocabQueueWidget({
  ...
  useEffect(() => {
    const handleManualPrefill = (event) => {
      const candidate = event?.detail?.candidate || event?.detail || {};
      const requestedCategory = normalizeCategoryValue(event?.detail?.category);
      if (requestedCategory) {
        setSelectedCategory(requestedCategory);
        setCategoryError('');
      }
      setMWord(String(candidate?.word || '').trim());
      setMContext(String(candidate?.context || '').trim());
      setMSource(String(candidate?.source || '混乱输入提取').trim());
      setMIntentionalBlank(!String(candidate?.context || '').trim());
      setActiveTab('manual');
      setQueueOpen(true);
    };

    window.addEventListener(PREFILL_VOCAB_MANUAL_EVENT, handleManualPrefill);
    return () => window.removeEventListener(PREFILL_VOCAB_MANUAL_EVENT, handleManualPrefill);
  }, [setQueueOpen]);
  ...
  const enqueueCandidate = useCallback((candidate, { fetchLlm = true } = {}) => {
    const category = String(selectedCategory || '').trim();
    if (!category) {
      setCategoryError('先选择目标目录。');
      return false;
    ...
    const newTask = {
      ...
      fetchLlm: Boolean(fetchLlm),
      focusPositions: [],
      intentionalBlank: !context,
      category,
      status: 'pending',
      error: null,
    };
  ...
  const handleUseCandidateInManual = useCallback((candidate) => {
    setMWord(String(candidate?.word || '').trim());
    setMContext(String(candidate?.context || '').trim());
    setMSource(String(candidate?.source || '混乱输入提取').trim());
    setMIntentionalBlank(!String(candidate?.context || '').trim());
  ...
  const panelNode = isOpen ? (
    <div className="queue-panel" style={{ width: embedded ? 'min(360px, calc(100vw - 28px))' : 'min(368px, calc(100vw - 28px))', height: embedded ? 'min(500px, calc(100vh - 104px))' : 'min(560px, calc(100vh - 116px))', background: '#fff', borderRadius: '6px', boxShadow: 'none', border: '1px solid var(--ms-border)', display: 'flex', flexDirection: 'column', marginBottom: embedded ? 0 : '12px', overflow: 'hidden' }}>
      ...
      <div className="queue-tabs" style={{ display: 'flex', borderBottom: '1px solid var(--ms-border)', background: 'var(--ms-surface-muted)' }}>
        ...
        <button className={`queue-tab-button${activeTab === 'extract' ? ' is-active' : ''}`} onClick={() => setActiveTab('extract')} style={{ flex: 1, padding: '10px 12px', border: 'none', background: activeTab === 'extract' ? '#fff' : 'transparent', fontWeight: activeTab === 'extract' ? '600' : '400', cursor: 'pointer', borderBottom: activeTab === 'extract' ? '2px solid var(--ms-text)' : '2px solid transparent', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
      ...
      {activeTab === 'extract' && (
        <VocabularyContextExtractPanel
          onUseCandidate={handleUseCandidateInManual}
          onEnqueueCandidate={(candidate) => enqueueCandidate(candidate, { fetchLlm: true })}
        />
      )}
