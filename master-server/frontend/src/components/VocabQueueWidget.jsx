...
import { addVocabulary, extractVocabularyCandidates } from '../api/client';
...
export default function VocabQueueWidget({
  ...
  const panelNode = isOpen ? (
    <div className="queue-panel" style={{ width: embedded ? 'min(360px, calc(100vw - 28px))' : 'min(368px, calc(100vw - 28px))', height: embedded ? 'min(500px, calc(100vh - 104px))' : 'min(560px, calc(100vh - 116px))', background: '#fff', borderRadius: '6px', boxShadow: 'none', border: '1px solid var(--ms-border)', display: 'flex', flexDirection: 'column', marginBottom: embedded ? 0 : '12px', overflow: 'hidden' }}>
      ...
      {activeTab === 'extract' && (
        <div className="queue-extract-panel">
            ...
            {extractCandidates.length ? extractCandidates.map((candidate, index) => (
              <div key={`${candidate.word}-${index}`} className="queue-extract-card">
                ...
                <div className="queue-extract-card-actions">
                  <button type="button" className="master-secondary-button" onClick={() => handleUseCandidateInManual(candidate)}>
                    录入
                  </button>
                  <button type="button" className="master-primary-button" onClick={() => enqueueCandidate(candidate, { fetchLlm: true })}>
                    加入队列
                  </button>
                </div>
              </div>
