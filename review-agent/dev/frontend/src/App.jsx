            ...
            <div className="example-editor-card" key={`example-${index}`}>
              ...
              <div className="editor-grid">
                ...
                <label>
                  ...
                  <input
                    className="field"
                    value={Array.isArray(example.focusPositions) ? example.focusPositions.join(', ') : ''}
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        <div className="panel-heading">
          <h3>复习建议与打分</h3>
          <div className="panel-caption">{current ? `${current.status} · 下次 ${current.next_review_date}` : '查看复习状态并直接记录本次评分'}</div>
        </div>
      ...
      <div className="panel-body">
        ...
        {!current ? (
          ...
          <>
            {hasRecordedReview ? (
              <div className="analysis-source">当前展示的是本次打分后的最新复习计划。</div>
            ) : null}
            <div className="status-grid">
              ...
              <div>
                <div className="card-title">最近一次记录</div>
                <div className="card-main">{current.last_review ? `${current.last_review.date} / ${current.last_review.score}` : '无'}</div>
                <div className="card-sub">累计 {current.review_count} 次</div>
              </div>
            </div>

            <div className="section-title">本次打分</div>
            ...
            {after && reviewData?.recorded_review ? (
              <div className="after-box">
                已记录 {reviewData.recorded_review.date} 分数 {reviewData.recorded_review.score}，
      ...
      <div className="panel-body sidebar-section sidebar-fill file-section">
        ...
        <label>
          ...
          <input
            ...
            disabled={!category || loadingFiles}
          />
        </label>

        <div className="file-list-meta">
          <span>{loadingFiles ? '词条加载中...' : `${filteredFiles.length} / ${files.length} 条`}</span>
          {fileQuery.trim() ? <span>过滤: {fileQuery.trim()}</span> : null}
        </div>

        <div className="nav-list nav-list-scroll">
          {!category ? <div className="empty">先选择一个目录。</div> : null}
          {category && !loadingFiles && !filteredFiles.length ? <div className="empty">没有匹配的词条。</div> : null}
      ...
      <div className="panel-body sidebar-section">
        ...
        {!current ? (
          ...
          >
            ...
            <div className="recommend-path">{current.category} / {current.file}</div>
            ...
            <div className="nav-list">
              {alternatives.map((item) => (
                ...
                >
                  ...
                  <span className="nav-sub">{item.category} / {item.file}</span>
                </button>
              ))}
            </div>
          </>
...
export default function App() {
  ...
  const entryPronunciation = useMemo(
    () => String(draft?.pronunciation || detail?.pronunciation || '').trim(),
    [detail, draft],
  );
  const youdaoUrl = useMemo(() => buildYoudaoUrl(activeWord), [activeWord]);

  const currentReview = reviewData?.after || reviewData?.before || null;
  ...
  const selectEntry = (nextCategory, nextFilename, source = 'manual') => {
    ...
    const normalizedFilename = String(nextFilename || '').trim();
    const changingCategory = Boolean(normalizedCategory && normalizedCategory !== category);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchVocabDetail(category, filename),
      getReviewAdvice(category, filename),
    ])
  ...
  }, [category, filename, hasSelection]);
  ...
  const handleUseRecommendation = (item) => {
    if (!item?.category || !item?.file) return;
  ...
  const refreshFiles = async () => {
    ...
    setLoadingFiles(true);
    try {
      const res = await fetchFiles(category);
      const list = res.files || [];
      setFiles(list);
  ...
  const handleMerge = async () => {
    ...
    try {
      setError('');
      const res = await runFolderRefine(category, includeLowConfidence, includeMergeLlm);
      setMergeData(res);
    } catch (err) {
      showError(err.message);
    } finally {
  ...
  const handleApplyMerge = async (item) => {
    if (!category || !item?.source?.file || !item?.target?.file) return;
    ...
    try {
      ...
      if (filename === targetFile) {
        void Promise.all([
          fetchVocabDetail(category, targetFile),
          getReviewAdvice(category, targetFile),
        ])
          .then(([detailRes, reviewRes]) => {
            hydrateDetailAndDraft(detailRes.data || null);
            setReviewData(reviewRes || null);
  ...
  const handleClean = async () => {
    ...
    try {
      ...
      const res = await runFileRefine(category, filename, includeLlm, analysisDraft);
      setCleanData(res);
  ...
  const handleApplyAllAndSave = async () => {
    ...
    try {
      ...
      const res = await saveVocabDetail(category, filename, payload);
      hydrateDetailAndDraft(res.data || payload);

      const refreshed = await runFileRefine(category, filename, false, res.data || payload);
  ...
  const handleReviewRefresh = async () => {
    ...
    try {
      setError('');
      const res = await getReviewAdvice(category, filename);
      setReviewData(res);
    } catch (err) {
      showError(err.message);
    } finally {
  ...
  const handleScore = async (score) => {
    ...
    try {
      setError('');
      const reviewRes = await submitReviewScore(category, filename, score, reviewDate);
      setReviewData(reviewRes);

      const detailRes = await fetchVocabDetail(category, filename);
      hydrateDetailAndDraft(detailRes.data || null);
    ...
    <div className="page workspace-page">
      <header className="topbar">
        <div className="brand-wrap">
          ...
          <div>
            <div className="brand">Review Agent</div>
        ...
        <div className="toolbar workspace-toolbar">
          ...
          <button className="ghost" onClick={refreshFiles} disabled={!category || loadingFiles}>
            {loadingFiles ? '刷新中...' : '刷新当前目录'}
          </button>
          <button className="primary" onClick={() => setDrawerOpen(true)}>配置</button>
        </div>
      </header>
            ...
            <div className="panel-body">
              <div className="workspace-summary-top">
                <div>
                  ...
                  {entryPronunciation ? <div className="hero-pronunciation">{entryPronunciation}</div> : null}
                </div>
              </div>

              <div className="hero-stats">
                <div className="stat-pill"><strong>{mode === 'manual' ? '手动' : '推荐'}</strong> 工作模式</div>
                <div className="stat-pill"><strong>{currentReview?.status || '--'}</strong> 复习状态</div>
                <div className="stat-pill"><strong>{currentReview?.next_review_date || '--'}</strong> 下次建议</div>
                <div className="stat-pill"><strong>{hasSelection ? filename.replace(/\.json$/i, '') : '--'}</strong> 当前文件</div>
              </div>
            </div>
          </section>
                ...
                <EditorPanel
                  ...
                  onExampleChange={(index, field, value) => updateDraft((base) => {
                    ...
                    if (field === 'focusWords') {
                      ...
                      example.focusPositions = uniqueInts(String(value || '').split(','));
                    } else {
                      example[field] = value;
                    }
