...
from services.llm import suggest_file_cleaning_with_llm, suggest_folder_merge_with_llm

router = APIRouter()
_WS_RE = re.compile(r"\s+")
logger = logging.getLogger("review_agent.api")
...
def get_config():
    ...
    return {
        ...
        "hasKey": config.get("hasKey", False),
    }
...
def get_categories():
    return {"status": "success", "categories": list_categories()}


@router.get("/api/vocabulary/list")
def get_vocab_list(category: str):
    try:
...
@router.get("/api/vocabulary/detail/{filename}")
def get_vocab_detail(filename: str, category: str):
    try:
...
def save_vocab(req: VocabSaveRequest):
    try:
        path, existing = load_vocab_entry(req.category, req.filename)
...
def refine_folder(req: FolderRefineRequest):
    try:
        ...
        files = list_vocab_files(req.category)
        entries = []
        skipped = []
...
def apply_merge(req: MergeApplyRequest):
    try:
        ...
        source_path, source_payload = load_vocab_entry(req.category, source_name)
        target_created = False
        if req.create_target_if_missing:
            target_path = resolve_vocab_file_for_write(req.category, target_name)
            ...
            target_path, target_payload = load_vocab_entry(req.category, target_name)
...
def refine_file(req: FileRefineRequest):
    try:
        ...
        path, payload = load_vocab_entry(req.category, req.filename)
        fallback_word = os.path.splitext(os.path.basename(path))[0]
